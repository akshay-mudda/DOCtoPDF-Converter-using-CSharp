...
namespace DOCtoPDF_Converter_using_CSharp
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            List<string> files = new List<string>(docFiles);
            files.AddRange(docxFiles);

            // Summary and move records
            ...
            foreach (string file in files)
            {
                try
                {
                    ...
                    string destFile = Path.Combine(destinationFolder, $"{fileName}.pdf");

                    // Convert DOC/DOCX to PDF
                    if (ConvertDocToPdf(file, destFile))
                    {
                        File.Delete(file);
                        Console.WriteLine($"Converted and moved {file} to {destFile}");
                        moveRecords.Add(new FileMoveRecord(fileName, "PDF", sourceFolder, destFile));
                    }
                }
                catch (Exception ex)
                {
        ...
        static bool ConvertDocToPdf(string sourceFile, string destinationFile)
        {
            try
            {
                Document document = new Document(sourceFile);
                document.Save(destinationFile, SaveFormat.Pdf);
                return true;
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Error converting file {sourceFile} to PDF: {ex.Message}");
                return false;
            }
        }
    }
